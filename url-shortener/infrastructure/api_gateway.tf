...
// get original url
// ================
resource "aws_api_gateway_resource" "original_url_root" {
  path_part   = "original-url"
  parent_id   = aws_api_gateway_rest_api.short_url.root_resource_id
  rest_api_id = aws_api_gateway_rest_api.short_url.id
}

resource "aws_api_gateway_resource" "original_url_path_variable" {
  depends_on = [
    aws_api_gateway_resource.original_url_root
  ]
  path_part   = "{hash}"
  parent_id   = aws_api_gateway_resource.original_url_root.id
  rest_api_id = aws_api_gateway_rest_api.short_url.id
}

resource "aws_api_gateway_method" "original_url_get" {
  rest_api_id   = aws_api_gateway_rest_api.short_url.id
  resource_id   = aws_api_gateway_resource.original_url_path_variable.id
  http_method   = "GET"
  authorization = "NONE"
